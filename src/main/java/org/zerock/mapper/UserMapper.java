...
public interface UserMapper {
	...
	//댓글 가져오기
	public List<CommentVO> commentselect (@Param("photo")List<PhotoVO> photo);
	
	//댓글 삭제하기
	public void  commentdelete(CommentVO vo);
	
	//유저아이디에 관련된 좋아요 리스트 가져오기
	public List<LikeVO> likeselect(@Param("photo")List<PhotoVO> photo, @Param("userid")String userid);
	
	//팔로우 되어있는지 확인
	public int followcheck(@Param("user_id")String user_id, @Param("follow_user")String follow_user);
	...
	public void updateprofile(@Param("user_photourl") String user_photourl 
			, @Param("user_photothumb") String user_photothumb
	...
	//좋아요 카운트 업데이트
	public void likecntupdate(@Param("pno") int pno);
	
	//좋아요 삭제 누르기
	public void likedelete(LikeVO lVo);
	
	//좋아요 카운트 업데이트
	public void likecntdelete(@Param("pno") int pno);
}